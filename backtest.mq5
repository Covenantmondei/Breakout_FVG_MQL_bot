...
void OnTick()
{
   ...
   if(UseNewsBlackout && InNewsBlackout())
   {
      if(EnableDebugPrints && tickCounter % DebugPrintInterval == 0)
         Print("📰 News blackout window – entries paused.");
      return;
   }
   ...
   if(fvgDirection != 0)
   {
      if(!PassesTrendSlope())
        {
            if(EnableDebugPrints && tickCounter % DebugPrintInterval == 0)
                PrintFormat("📉 MA slope %.1f < %.1f", CurrentMaSlopePoints(), TrendSlopeMinPoints);
            return;
        }
        ...
        {
            if(EnableDebugPrints && tickCounter % DebugPrintInterval == 0)
                PrintFormat("🧭 ADX %.1f < %.1f", CurrentAdxValue(), AdxTrendingThreshold);
            return;
        }

      if(EnforceDirectionalCap && CountDirectionalTrades(fvgDirection) >= MaxTradesPerDirection)
      {
         if(EnableDebugPrints && tickCounter % DebugPrintInterval == 0)
            Print("🚫 Directional trade cap reached.");
         return;
      }
...
void PlaceMarketTrade(int direction, double highRange, double lowRange, double atr)
{
   ...
   if(avgFvgHeight > 0.0)
      slDistance = MathMin(slDistance, avgFvgHeight * 1.5);

   if(UseDynamicAtrStops && atr > 0.0)
      slDistance = MathMax(slDistance, atr * DynamicSlAtrMult);
   ...
   double tp;
   if(UseDynamicAtrStops && atr > 0.0)
      tp = (direction == 1) ? (entry + atr * DynamicTpAtrMult)
                            : (entry - atr * DynamicTpAtrMult);
   else
      tp = (direction == 1) ? (entry + risk * RR)
                            : (entry - risk * RR);
...
double CurrentMaSlopePoints()
{
   if(maHandle == INVALID_HANDLE) return 0.0;
   double buf[2];
   if(CopyBuffer(maHandle, 0, 0, 2, buf) != 2) return 0.0;
   double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   return (point > 0.0) ? (buf[0] - buf[1]) / point : (buf[0] - buf[1]);
}

double CurrentAdxValue()
{
